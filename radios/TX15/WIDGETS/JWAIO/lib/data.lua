...
return function(config, util)
  ...
  function M.update(state, options)
    ...
    -- Resoudre le nom en identifiant : ne pas masquer une source inconnue
    -- par un faux ACRO. Reessayer si la source n'etait pas encore disponible.
    if not state.modeSourceId or state.modeSourceId == 0 then
      state.modeSourceId = util.sourceIndex(string.lower(config.modeSource or ""))
    end
    local okMode, modeValue = false, nil
    if state.modeSourceId ~= 0 then
      okMode, modeValue = pcall(getValue, state.modeSourceId)
    end
    state.modeValid = okMode and type(modeValue) == "number" and
      modeValue == modeValue and math.abs(modeValue) <= 2048
    state.modeRaw = state.modeValid and modeValue or nil
    if state.rth then
      state.mode = "RTH"
    elseif state.modeValid then
      local position = modeValue < config.modeLow and -1 or
        (modeValue > (config.modeHigh or 341) and 1 or 0)
      state.mode = position == config.modeAnglePosition and "ANGLE" or "ACRO"
    else
      state.mode = "NO_DATA"
    end
    ...
    if state.now >= state.nextNavigationUpdate then
      ...
      state.satelliteLevel = nil
      if state.satsValid and state.sats >= 0 then
        state.satelliteLevel = state.sats < config.gpsReadySatellites and 1 or
          (state.sats < config.gpsRescueSatellites and 2 or 3)
      end
