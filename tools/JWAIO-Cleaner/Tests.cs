...
class Tests {
 ...
 static void Main(string[] args){
  ...
  string nested=Fixture(parent,"nested");Put(nested,"JWAIO-old-copy/WIDGETS/JWAIO/main.lua","local BASE = '/WIDGETS/JWAIO/'");Put(nested,"WIDGETS/JWAIO-old/main.lua","local BASE = '/WIDGETS/JWAIO/'");var np=Engine.Scan(nested,true,true);Assert(np.Found.Any(x=>x.Contains("JWAIO-old-copy"))&&np.Found.Any(x=>x.Contains("JWAIO-old")),"nested archive and renamed old widget detected");Engine.Execute(np);Assert(!File.Exists(Path.Combine(nested,"WIDGETS/JWAIO-old/main.lua")),"renamed old widget removed");
